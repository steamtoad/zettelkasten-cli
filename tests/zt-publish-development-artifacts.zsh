...
mkdir -p "$destination/dev/skills"
print -r -- 'stale' > "$destination/dev/skills/stale.txt"
...
[[ ! -e "$destination/dev/skills/stale.txt" ]] || { print -u2 -- 'FAIL: skills mirror did not delete stale file'; exit 1; }
...
cmp -s "$repo/dev/skills/zettelkasten-development/SKILL.md" "$destination/dev/skills/zettelkasten-development/SKILL.md" || { print -u2 -- 'FAIL: development skill differs after publish'; exit 1; }
