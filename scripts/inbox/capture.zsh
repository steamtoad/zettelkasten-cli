#!/bin/zsh

#------------------------------------------------------------------------------
# ZT-Inbox
# Тип: inbox utility
# Назначение: создать новый элемент в inbox/raw
#------------------------------------------------------------------------------

emulate -L zsh
setopt nounset

zmodload zsh/system || {
    print -u2 "Error: zsh/system is required"
    exit 1
}

SCRIPT_DIR="${0:A:h}"
ZK_ROOT="${ZK_HOME:-${ZETTELKASTEN_ROOT:-${SCRIPT_DIR:h:h}}}"

INBOX_DIR="${ZK_ROOT}/inbox"
RAW_DIR="${INBOX_DIR}/raw"

EDITOR_CMD="${EDITOR:-vim}"

usage() {
    print -u2 "Usage: zt-inbox.zsh \"Title\""
}

die() {
    print -u2 "Error: $*"
    exit 1
}

[[ $# -ge 1 ]] || {
    usage
    exit 1
}

title="$*"

[[ -n "${title//[[:space:]]/}" ]] || die "title must not be empty"
[[ "$title" != *$'\n'* && "$title" != *$'\r'* ]] ||
    die "title must be a single line"

mkdir -p "$RAW_DIR" || die "cannot create directory: $RAW_DIR"

timestamp="$(date '+%Y-%m-%d-%H%M%S')"
captured_at="$(date '+%Y-%m-%dT%H:%M:%S%z')"

sequence=0

# Изолированный cwd обходит ошибку sysopen в системном Zsh macOS на Unicode
# полном пути. ASCII basename и O_EXCL сохраняют защиту от конкурентной записи.
capture_file() (
    builtin cd -- "$RAW_DIR" || return 1
    if ! sysopen -w -o creat,excl -u capture_fd "$filename" 2>/dev/null; then
        [[ -e "$filename" || -L "$filename" ]] && return 2
        return 1
    fi

    if ! {
        print -u "$capture_fd" -r -- "= ${title}" &&
        print -u "$capture_fd" -r -- ":captured-at: ${captured_at}" &&
        print -u "$capture_fd" -r -- ":source: manual" &&
        print -u "$capture_fd" -r -- ""
    }; then
        exec {capture_fd}>&-
        rm -f -- "$filename"
        return 3
    fi
    exec {capture_fd}>&-
)

while true; do
    if (( sequence == 0 )); then
        filename="${timestamp}.adoc"
    else
        filename="${timestamp}-${sequence}.adoc"
    fi

    filepath="${RAW_DIR}/${filename}"

    capture_file
    capture_rc=$?
    case "$capture_rc" in
        0) break ;;
        2) (( sequence++ )) ;;
        3) die "cannot write file: $filepath" ;;
        *) die "cannot create file: $filepath" ;;
    esac
done

[[ -f "$filepath" ]] || die "cannot create file: $filepath"

editor_words=("${(@Q)${(z)EDITOR_CMD}}")
(( ${#editor_words[@]} > 0 )) || die "EDITOR must not be empty"
command -v "${editor_words[1]}" >/dev/null 2>&1 ||
    die "editor not found: ${editor_words[1]}"

print -r -- "$filepath"

exec "${editor_words[@]}" "$filepath"
