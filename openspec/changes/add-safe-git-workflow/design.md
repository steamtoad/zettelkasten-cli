# Design: Явный безопасный Git workflow

## Context

Git хранит историю, но нет CLI preflight конфликтов и безопасного sync; publisher не является Git update. Основание: `audit/2026-09-05/7b3fcec8-a907-11f1-8a46-53bdf0fb80b5.adoc` (G12, G08). Это целевой контракт, а не заявление об устранённом дефекте.

## Goals / Non-Goals

Цель: Дать read-only status/conflicts и явный fast-forward sync без уничтожения локальных изменений.
...
zt-git status/conflicts работают локально read-only. zt-git sync — отдельное явное сетевое действие по configured upstream; scheduler/автообновление не включается по умолчанию.

### 2. Решение

При dirty index/worktree, untracked collisions, merge/rebase/cherry-pick in progress, detached HEAD, missing upstream или divergence sync прекращается. Нет autostash, reset, rebase, force push или автоматического выбора стороны конфликта.

### 3. Решение

Конфликтный helper показывает файл .adoc и base/ours/theirs для ручного разбора, не разрешает конфликт сам. Fetch меняет Git refs, но не разрешает push.
