# Проверка refactor-workspace-as-plugin
...
Проверены validation title, безопасное имя, collision, точный template, отмена выбора, active-type filtering, multi-select, idempotent add, read-only open. Remove проверен на active/deprecated/broken targets, всех восьми поддерживаемых delimiters блоков, отмене, ошибке до замены файла и сохранении mode 0640. Bytes связанных документов остаются неизменными.

До переноса behavioral часть `tests/zt-workspace-as-plugin.zsh --behavior-only` прошла на исходной реализации. Structural assertions отклонили исходную архитектуру. После переноса полный тест проходит через top-level entrypoints, включая точное forwarding аргументов, stdout/stderr, exit 37 и прежний managed skill mapping. Все операции выполняются в disposable fixtures.
...
- `openspec validate refactor-workspace-as-plugin --strict --no-interactive` — PASS.
