# Проверка refactor-diary-as-plugin
...
Проверены UUID v1, обязательные metadata, basename `.last-diary`, previous/next links, точное содержимое `all-todays`, Vim argument и stdout. Проверены invalid state, ошибка UUID, невозможность регистрации today и ошибка записи next link до обновления state. Сохранён прежний результат при ненулевом завершении Vim. Neutral constructor не изменён.

До переноса behavioral часть `tests/zt-diary-as-plugin.zsh --behavior-only` прошла на исходной реализации. Structural assertions отклонили исходную архитектуру. После переноса полный тест проходит через top-level entrypoints, включая точное forwarding аргументов, stdout/stderr, exit 37 и прежний managed skill mapping. Все операции выполняются в disposable fixtures.

Проверки общей реализации:

- `zsh -n` для Zsh sources и tests — PASS.
- `tests/zt-diary-as-plugin.zsh` — PASS.
...
- `openspec validate refactor-diary-as-plugin --strict --no-interactive` — PASS.
