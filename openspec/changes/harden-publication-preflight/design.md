# Design: Безопасное зеркало development-поставки

## Context

rsync --delete применяется без проверки равных/вложенных путей, dirty destination и изменения после просмотренного плана. Основание: `audit/2026-09-05/7b3fcec8-a907-11f1-8a46-53bdf0fb80b5.adoc` (G08). Это целевой контракт, а не заявление об устранённом дефекте.

## Goals / Non-Goals

Цель: Сохранить явное exact mirror, но разрешать его только по актуальному плану с проверенными границами и восстановлением.
...
Default dry-run и существующий --apply сохраняются. Для noninteractive apply необходим идентификатор сохранённого preview; interactive --apply может построить и подтвердить текущий план в одной сессии. Непросмотренный автоматический mirror запрещён.

### 2. Решение

Canonicalized source/destination не равны, не вложены друг в друга, не содержат symlinked publish subtrees за их границами. Проверяется только явный allowlist .scripts/openspec/skills/tests и root files.

### 3. Решение

Git destination должен быть clean; для destination без Git допускается пустая папка либо совпадение с предыдущим publication manifest. Произвольная непустая unmanaged папка отклоняется.

### 4. Решение

Артефакты backup/manifest пишутся вне зеркалируемых деревьев назначения; rollback не использует git reset.
