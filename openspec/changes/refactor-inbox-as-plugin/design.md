...
Plugin продолжает вычислять пути от `ZK_HOME` и использовать `inbox/raw` и `inbox/processed`. Установка не запускает migration hook и не сканирует эти каталоги. Legacy traceability переносит ownership актуальных Inbox requirements в plugin documentation без изменения стабильных IDs или статуса; `ROADMAP-004` остаётся ROADMAP до отдельного изменения, реализующего import.
