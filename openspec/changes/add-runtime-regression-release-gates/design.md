# Design: Поведенческая регрессия и проверяемая зрелость релиза

## Context

Зелёные проверки 316 ID не доказывают runtime; нет полной матрицы отказов, Linux/macOS и чистой поставки. Основание: `audit/2026-09-05/7b3fcec8-a907-11f1-8a46-53bdf0fb80b5.adoc` (G11, G01, G02, G03, G04, G05, G06, G07, G08, G09, G10). Это целевой контракт, а не заявление об устранённом дефекте.

## Goals / Non-Goals

Цель: Связать runtime-гарантии с executable tests, платформами и release evidence; не выдавать SKIP за PASS.
...
У каждого change собственный primary runtime test, имя которого совпадает с change ID: tests/zt-<change-id>.zsh. Матрица агрегирует результат, а не подменяет его text grep.

### 2. Решение

Required regression coverage — все создание/привязки, Diary/Continue, Clean/Full Copy/repeated Reduce, обе archive-source ветви Refine, Inbox, Workspace, migration, publication, failure/recovery.

### 3. Решение

CI использует macOS и Linux с настоящими системными UUID providers и zsh; scripted fzf/editor заменяют только интерфейс. Missing optional OpenClaw не мешает portable suite.

### 4. Решение

Совместный spec preview синхронизирует только реализуемые deltas без потери sibling requirements. Перемещение плагинов из уже открытых изменений проверяется на обновлённом behavioral baseline.

### 5. Решение

Тест архивного governance не привязывается навечно к active change path: после archive работает с canonical requirements и isolated historical fixture. Existing add-openspec-archive-contract остаётся единственным владельцем archive policy.
