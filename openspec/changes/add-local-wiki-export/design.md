# Design: Производная локальная wiki из AsciiDoc

## Context

zt-wiki отсутствует; нет формального безопасного контракта для производного просмотра знаний. Основание: `audit/2026-09-05/7b3fcec8-a907-11f1-8a46-53bdf0fb80b5.adoc` (G12). Это целевой контракт, а не заявление об устранённом дефекте.

## Goals / Non-Goals

Цель: Опционально экспортировать локальный HTML-view без превращения его в source of truth и без публикации.
...
zt-wiki export требует явный output directory вне Vault и source repo, Asciidoctor как optional dependency и dry-run по умолчанию. --apply пишет только выбранный локальный output.

### 2. Решение

Default export включает только активные persistent documents. Ссылки на исключённые архивные объекты показываются как неэкспортированные исторические references, не расширяют export scope.

### 3. Решение

Include/macros не должны читать за пределами выбранной области или выполнять команды. Неподдержанное разрешение ссылки/unsafe include блокирует export с diagnostics. Применяются staging и output manifest, без очистки чужой папки.
