# Design: Полный граф входящих ссылок при миграции notes

## Context

Миграция меняет ссылки только в moved documents, all-todays и workspaces; корневой overview остаётся сломанным. Основание: `audit/2026-09-05/7b3fcec8-a907-11f1-8a46-53bdf0fb80b5.adoc` (G06). Это целевой контракт, а не заявление об устранённом дефекте.

## Goals / Non-Goals

Цель: Применять проверенный manifest всех поддерживаемых входящих ссылок, а неподдерживаемые случаи диагностировать до записи.
...
Scan включает обычные .adoc внутри явно выбранного ZK_HOME, кроме .git, служебной .state и .scripts. Пропуск любого plugin namespace согласуется с его владельцем; knowledge/ учитывается при наличии.

### 2. Решение

Внешние источники ссылок вне выбранного Vault не обнаруживаются автоматически: preview явно указывает границу гарантии. Symlink наружу не обходится для mutation.

### 3. Решение

Технический первый этап может отказать при неподдерживаемом источнике. Пакет завершён только когда поддержанные корневые overview и plugin reference links преобразуются и проходят postflight.
