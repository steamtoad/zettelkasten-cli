# Design: Явное разрешение и диагностика Vault

## Context

Нет отдельной диагностики выбранного data vault и отличия от source repository. Основание: `audit/2026-09-05/7b3fcec8-a907-11f1-8a46-53bdf0fb80b5.adoc` (G12, G07). Это целевой контракт, а не заявление об устранённом дефекте.

## Goals / Non-Goals

Цель: Read-only zt-vault path/status/check показывает область данных и ошибки структуры без скрытого создания или переноса.
...
Первый zt-vault — инспекция, не registry и не migration manager. Явный --root имеет приоритет над ZK_HOME, затем используется прежний zk_home fallback.

### 2. Решение

Не добавляются default vault switching, разделение Memo или разбиение all-todays по годам. Будущие move/init/registry требуют отдельных проверенных deltas.
